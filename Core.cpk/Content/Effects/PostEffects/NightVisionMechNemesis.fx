...
{     
	...
	// make color grayscale but use only the blue component
    float c = dot(modifiedColor.rgb, float3(0.3, 0.59, 0.11));
    modifiedColor.r = 0.4 * (c + AdditionalLight);
    modifiedColor.b = c + AdditionalLight;
	modifiedColor.g = 0;
    
    modifiedColor = lerp(originalColor, modifiedColor, 0.5);
		
	// apply vignetting
	float vignettingCoef = 1 - distance(float2(0.5, 0.5), input.TexCoord);
	vignettingCoef = pow(vignettingCoef, 0.667);
	vignettingCoef = 2 * (vignettingCoef - 0.44);
