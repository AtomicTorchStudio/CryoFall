namespace AtomicTorch.CBND.CoreMod.StaticObjects.Vegetation.Trees
{
    ...
    public class ObjectTreePineArctic : ProtoObjectTree
    {
        public override string Name => GetProtoEntity<ObjectTreePine>().Name;

        public override double TreeHeight => 2.2;

        protected override TimeSpan TimeToMature => TimeSpan.FromHours(6);
        ...
        {
            data.PhysicsBody
                .AddShapeRectangle(size: (0.8, 0.6),  offset: (0.1, 0.4))
                .AddShapeRectangle(size: (0.8, 1.2),  offset: (0.1, 0.4), group: CollisionGroups.HitboxMelee)
                .AddShapeRectangle(size: (0.4, 0.55), offset: (0.3, 1.1), group: CollisionGroups.HitboxRanged);
        }
    }
}